        ...
        <div class="card-body">
          <form v-on:submit.prevent class="form-horizontal" >
        	...
        	<small>{{errors.last_updated_error}}</small>
				...
				<button @click="updateDoc()" type="submit" class="btn btn-primary">Submit</button>
          </form>
        </div>
    </div>
</div>
...
created : function(){
  ...
  axios.get(window.location.origin+'/api/docs/'+this.$route.params.id,headers).then((response)=>{
...
methods:{
  updateDoc:function(){
    ...
    axios.put(window.location.origin+'/api/docs/'+this.$route.params.id,doc,{headers:headers}).then((response)=>{
