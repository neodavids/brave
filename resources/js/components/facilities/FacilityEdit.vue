        ...
        <div class="card-body">
          <form v-on:submit.prevent class="form-horizontal" >
        	...
        	<small>{{errors.description_error}}</small>
				...
				<button @click="updateFacility()" type="submit" class="btn btn-primary">Submit</button>
          </form>
        </div>
    </div>
</div>
...
created : function(){
  ...
  axios.get(window.location.origin+'/api/facilities/'+this.$route.params.id,headers).then((response)=>{
...
methods:{
  updateFacility:function(){
    ...
    axios.put(window.location.origin+'/api/facilities/'+this.$route.params.id,facility,{headers:headers}).then((response)=>{
