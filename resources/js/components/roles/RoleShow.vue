<template>
  <div>
      <div v-if="role!=null" id="role">
        ...
        <div>
          <router-link :to="'/roles/'+$route.params.slug+'/edit'" class="btn btn-primary">Edit</router-link>
          <button @click="deleteRole(role)" :to="'/roles/'+$route.params.slug" class="btn btn-danger">Delete</button>
        </div>
      </div>
    </div>
  </div>
...
methods:{
  fetchRole:function(){
    ...
    axios.get(window.location.origin+'/api/roles/'+slug).then((response)=>{
